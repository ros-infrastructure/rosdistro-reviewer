...
@pytest.mark.linter
def test_spell_check(known_words) -> None:
    ...
    source_filenames = [Path(__file__).parents[1] / 'setup.py'] + \
        list(
            (Path(__file__).parents[1] / 'rosdistro_reviewer')
            .glob('**/*.py')) + \
...
@pytest.mark.linter
def test_spell_check_word_list_order(known_words) -> None:
    assert known_words == sorted(known_words), \
        'The word list should be ordered alphabetically'


@pytest.mark.linter
def test_spell_check_word_list_duplicates(known_words) -> None:
